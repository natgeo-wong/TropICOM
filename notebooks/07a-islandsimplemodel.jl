...
md"
# 7a. Simple Tropical Island Models
...
begin
	...
	const Fs_sst300 = zeros(nεsw,nεlw,nα)
...
for iα = 1 : nα, iεlw = 1 : nεlw, iεsw = 1 : nεsw
	...
	Fs_sst300[iεsw,iεlw,iα] = (1-αvec[iα]) * (1-εswvec[iεsw]) * 433.22 - σ * 300^4 + σ * ta_sst300[iεsw,iεlw,iα]^4
end
...
begin
	...
	c = axs[1].contourf(
		εswvec,εlwvec,ta_sst300[:,:,1]'.-250,
		levels=vcat(-100,-50,-20,-10,-5,-2,-1,1,2,5,10,20,50,100)/10,
		extend="both",cmap="negpos"
	)
	axs[1].format(ylim=(0.9,1),xlim=(0,0.1),ltitle=L"(a) $T_a$ - 250 / K")

	c1_2 = axs[2].contourf(
		εswvec,εlwvec,1.181 * ta_sst300[:,:,1]' * 287 / 9.81 * 0.7/1000,
		levels=(6:0.02:6.2),extend="both"
	)
	axs[2].format(ylim=(0.9,1),xlim=(0,0.1),ltitle=L"(b) $c_{p.a}$ / MJ kg$^{-1}$ K$^{-1}$")

	c1_3 = axs[3].contourf(
		εswvec,εlwvec,Fs_sst300[:,:,1]',
		levels=150:5:200,extend="both"
	)
	axs[3].format(ylim=(0.9,1),xlim=(0,0.1),ltitle=L"(c) $F_s$ / W s$^{-1}$")
...
# ╔═╡ 0f425a11-1398-4b81-a4f1-0719161494a3
md"
All of these equations have been put into a module called `SimpleIslandModels` that is a subpackage of this TroPrecLS project.
"
