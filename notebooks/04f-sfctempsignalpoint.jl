...
begin
	...
	for idt in e5ds.start : Month(1) : e5ds.stop
		...
		ndy  = daysinmonth(idt)
		sstii = @view sstdy[:,:,ibeg:iend]
		ids = NCDataset(ERA5Reanalysis.e5dh2d(e5ds,evar_sst,egeo,idt))
		NCDatasets.load!(ids[evar_sst.varID].var,sstii,:,:,:)
	end
end
...
begin
	...
	for ilat = 1 : length(lsd.lat), ilon = 1 : length(lsd.lon)
		pdg = periodogram(sstdy[ilon,ilat,:],fs=365)
		a2[1].plot(pdg.freq,pdg.power,lw=1,c="k")
	end
	a2[1].format(
...
begin
	...
	itt = (0 : 0.001 : 86) * pi
	mann = 0.25*sin.(itt.-pi/2)
	msan = - 0.30*sin.(itt.*2)
	mitr = 0.1*sin.(itt.*12).+0.1*sin.(itt.*20).+0.1*sin.(itt.*28)
