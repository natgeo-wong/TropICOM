...
md"
# 3b. Total Column Water Vapour Data
...
begin
	...
	creg = areg[1].contourf(rlon,rlat,rμ',levels=15:5:75,cmap="Blues",extend="both")
	areg[1].plot(x,y,c="k",lw=0.5)
	areg[1].format(urtitle=L"$\mu$ / K")
	areg[1].colorbar(creg,loc="r")
	
	creg = areg[2].contourf(rlon,rlat,rA',levels=10. .^(-1:0.2:1),extend="both")
	...
	freg.savefig(plotsdir("tcwvmodel_reg.png"),transparent=false,dpi=200)
	PNGFiles.load(plotsdir("tcwvmodel_reg.png"))
end
