...
function compile(
    ...
    prcp = zeros(Float32,nlon,nlat,48,12)
    cnt  = zeros(Float32,nlon,nlat,48,12)
    ...
    for dt in dtvec
        ...
        for it = 1 : 48, ilat = 1 : nlat, ilon = 1 : nlon
            ...
            if !isnan(tmpii)
                prcp[ilon,ilat,it,mo] += tmpii
                cnt[ilon,ilat,it,mo]  += 1
            end
        end

    end

    for imo = 1 : 12, it = 1 : 48, ilat = 1 : nlat, ilon = 1 : nlon
        prcp[ilon,ilat,it,imo] = prcp[ilon,ilat,it,imo] / cnt[ilon,ilat,it,imo]
    end

    @info "$(now()) - TroPrecLS - Saving compiled GPM IMERG Half-Hourly data in Tropics GeoRegion ..."

    fnc = datadir("imergfinalmh-$(geo.regID)-compiled.nc")
    if isfile(fnc)
        rm(fnc,force=true)
    end
	...
	ncprcp = defVar(ds,"precipitation",Float32,("longitude","latitude","time","month"),attrib = Dict(
	    "units"     => "kg m**-2 s**-1",
