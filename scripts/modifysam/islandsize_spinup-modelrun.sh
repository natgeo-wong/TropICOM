...
prmfile=$exproot/prm/IslandSize/$experiment/spinup-${config}.prm
...
for fbin3D in *spinup-${experiment}-${config}*.bin3D
do
    rm "$fbin3D"
done

for fbin2D in *spinup-${experiment}-${config}*.bin2D
do
    rm "$fbin2D"
done
...
for f2Dbin in *spinup-${experiment}-${config}*.2Dbin
do
    rm "$f2Dbin"
done
...
for fstat in *spinup-${experiment}-${config}*.stat
do
    rm "$fstat"
done
